...
---------------------------------------------------------------------------------------------------------------------*/
    $('.sortable').each(function(i)
    {
        ...
        if ($($this).attr('search') === 'false') {
            dom = '<"block-controls"<"controls-buttons"p>>rti';
        }
        if ($($this).attr('display_record') === '11') {
            $display_record = 11;
        }
        if ($($this).attr('footer') === 'false') {
            dom = '<"block-controls"<"controls-buttons"p> <"filter"f>>rti';
        }
        else {
            dom = '<"block-controls"<"controls-buttons"p> <"filter"f>>rti<"block-footer clearfix"l>';
        }
        if ($($this).attr('page') === 'setting') {
            dom = '<"block-controls"<"controls-buttons"> <"filter"f>>rti';
        }
        if ($($this).attr('page') === 'sysformat_showview') {
            dom = '<"block-controls"<"controls-buttons"> <"filter"f>>rti';
        }
        if ($($this).attr('page') === 'idx_page' && $($this).attr('tab') == 'news') {
            scrollY = '395px';
            $display_record = 11;
        }
        if ($($this).attr('page') === 'upload') {
            dom = '<"block-controls"<"controls-buttons"> <"filter"f>>rt';
        }
        var oTable = table.dataTable({
            ...
            aoColumns: $data,
            "sScrollY": $($this).attr("pagination") == "false" && $display_record == 10 ? "280px" : scrollY,
            "bScrollCollapse": $($this).attr("pagination") == "false" ? true : false,
            "bPaginate": $($this).attr("pagination") == "false" ? false : true,
            "aaSorting": [],
            "iDisplayLength": $display_record,
            "iDisplayStart": 0,
