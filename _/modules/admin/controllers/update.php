...
class Update extends Admin {
    ...
    {
        $this->template->write_view('content', 'update/upload_data_forday', $this->data);
        $this->template->write('title', 'Upload Data');
        $this->template->render();
    }
	public function clean_shares_upc()
	{
	$list_data = $this->db->query("select DISTINCT ticker from shares_upc where new_shou is null;")->result_array();
		foreach ($list_data as $ticker)
			{
				$this->db->query("drop table if exists tmp;");
				$this->db->query("create  table  tmp select * from shares_upc where  ticker='".$ticker['ticker']."' order by date ;");
				$this->db->query("set @date1=(select min(date) as startdate from tmp where shli<>0 and new_shou is null);");
				$this->db->query("set @date2=(select max(date) as enddate from tmp where shli<>0 and new_shou is null);");
				$this->db->query("set @date3=(select min(date) as enddate from tmp where shli<>0 and new_shou is not null);");

				$this->db->query("set @shli1=(select shli from tmp where date=@date1);");
				$this->db->query("set @shli2=(select shli from tmp where date=@date2);");
				$this->db->query("set @shou=(select new_shou from tmp where date=@date3);");

				$this->db->query("update tmp set new_shou=if(@shli1=@shli2,@shou,null) where date>=@date1 and date<=@date2;");
				$this->db->query("update shares_upc as a, tmp set a.new_shou=tmp.new_shou, a.shli=tmp.shli 
				where a.ticker=tmp.ticker and a.date= tmp.date and a.market=tmp.market;");
			}
	$this->clean_shares_upc2();
	$this->clean_shares_upc3();
	echo "Done!";
	}
		public function clean_shares_upc2()
	{
	$list_data = $this->db->query("select DISTINCT ticker from shares_upc where new_shou is null;")->result_array();
		foreach ($list_data as $ticker)
		{
			$this->db->query("drop table if exists tmp;");
			$this->db->query("create  table  tmp select * from shares_upc where  ticker='".$ticker['ticker']."' order by date ;");
			$this->db->query("set @date1=(select min(date) as startdate from tmp where shli<>0 and new_shou is not null);");
			$this->db->query("set @shli1=(select shli from tmp where date=@date1);");
			$this->db->query("set @shou=(select new_shou from tmp where date=@date1);");
			$this->db->query("update tmp set new_shou=if(@shli1=@shou,shli,null) where  date<=@date1;");
			$this->db->query("update shares_upc as a, tmp set a.new_shou=tmp.new_shou, a.shli=tmp.shli where a.ticker=tmp.ticker and a.date= tmp.date and a.market=tmp.market;");
		}
		//echo "Done!";
	}
	public function clean_shares_upc3()
	{
	$list_data = $this->db->query("select DISTINCT ticker from shares_upc where new_shou is null;")->result_array();
		foreach ($list_data as $ticker)
		{
			...
			$this->db->query("set @shli1=(select shli from tmp where date=@date1);");
			$this->db->query("set @shou=(select new_shou from tmp where date=@date1);");
			$this->db->query("update tmp set new_shou=if(@shli1 is null,shli,null) where  new_shou is null;");
			$this->db->query("update shares_upc as a, tmp set a.new_shou=tmp.new_shou, a.shli=tmp.shli where a.ticker=tmp.ticker and a.date= tmp.date and a.market=tmp.market;");
		}
		//echo "Done!";
	}
    ...
    public function update_idx_compo()
    {
        if ($this->input->is_ajax_request())
        {
            ...
            $this->db->query("delete from `ifrcdata_db`.`idx_compo` where left(code,3) IN ('VNX')");
...
");
            $this->db->query("delete from vnxindex_data.obs_home where provider not in('VNX','IFRC','IFRCLAB','HOSE','HNX','PROVINCIAL');");
...
from ifrcdata_db.vndb_prices_history  date >='2008-12-31' 
...
FROM vnxindex_data.stk_month_chart a,(SELECT @runtot:=0, @runtot1:=0) c ");
    ...
    public function update_idx($dateChoice)
    {
        ...
        $dataCodeDay = $this->db->query('SELECT DISTINCT(code) FROM `vnxindex_data`.`idx_day` WHERE provider IN ("VNX","IFRC","IFRCLAB")')->result_array();
...
WHERE A.`code`=B.`code` AND A.`date`=B.`date` AND A.`dvar` = 0');
    ...
    public function idx_day($dateChoice)
    {
        ...
        $this->db->query('DELETE FROM `vnxindex_data`.`idx_day` WHERE 
                `date` >= "' . $dateChoice . '" AND ( provider in("IFRC") or  provider="")');
...
WHERE A.code = B.code');            
    ...
    public function update_currency_index()
    {
            set_time_limit(0);
            ini_set('memory_limit', '5000M');
            $from = microtime(true);
            $dataResultCode = $this->db->query('SELECT code FROM `vnxindex_data`.`idx_sample` WHERE provider = "IFRC" and type = "CURRENCY"')->result_array();
            $dataCode = array();
            foreach ($dataResultCode as $code)
            {
                $dataCode[] = $code['code'];
            ...
            $this->db->query('SET @plr1 = NULL');
            $this->db->query('DROP TABLE IF EXISTS tmp');
            $this->db->query('CREATE TEMPORARY TABLE tmp SELECT * FROM `vnxindex_data`.`idx_day` where provider="IFRC" and `code` like "INDCUR%" ORDER BY code, date');
            $this->db->query('DROP TABLE IF EXISTS tmp2');
            $this->db->query('CREATE TEMPORARY TABLE tmp2
            SELECT code, date, close,
            IF(@plr = code,(@runtot := ((a.close/ @runtot)-1)*100),null) AS perform,
    ...
    public function update_get_hnx()
    {
        if ($this->input->is_ajax_request())
        {
            ...
            $arrCode = array(
				...
				'HNXUPCoMPremium' => 'IFRCUPCOMPREMIUM'
            );
...
echo $content;
    ...
    public function upload_data_forday()
    {
        if ($this->input->is_ajax_request())
        {
            $from = microtime(true);
