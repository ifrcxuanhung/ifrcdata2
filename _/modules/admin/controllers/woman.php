...
class Woman extends Admin {
    ...
    public function download_ceo_part($date){
        ...
        //while(1){
            ...
            $dataTicker = $this->db->query('SELECT ticker, `name`, market FROM `vndb_reference_daily` WHERE date = (SELECT max(date) as date FROM `vndb_reference_daily`) limit 10;')->result_array();
...
public function download_ceo_part_age($date){
        $date = date('Y-m-d');
        header('Content-Type: text/html; charset=utf-8');
        $this->db->query('TRUNCATE TABLE all_women_caf_age_tmp');
        $options = array(
            CURLOPT_RETURNTRANSFER => true, // return web page
            CURLOPT_FOLLOWLOCATION => true, // follow redirects
            CURLOPT_USERAGENT => "phuong", // who am i
            CURLOPT_AUTOREFERER => true, // set referer on redirect
            CURLOPT_CONNECTTIMEOUT => 120, // timeout on connect
            CURLOPT_TIMEOUT => 120, // timeout on response
            CURLOPT_MAXREDIRS => 10, // stop after 10 redirects
        ...
        curl_setopt_array( $ch, $options );
            ...
            $dataTicker = $this->db->query('SELECT ticker, `name`, market FROM `vndb_reference_daily` WHERE date = (SELECT max(date) as date FROM `vndb_reference_daily`) limit 2;')->result_array();
            if(count($dataTicker) == 0){
                break;
            }else{
                $dataUpdate = array();
                foreach($dataTicker as $ticker){
                    ...
                    $start = '<table class="cl_ceo" cellspacing="0" cellpadding="0" border="0" style="width: 100%">';
                    $end = '</table>';
                    //PartitionString($start, $end, $content);
                    $start1 = preg_quote($start, '/t');
                    $end1 = preg_quote($end, '/t');
                    $rule = "/(?<=$start1).*(?=$end1)/msU";
                    $result = array();
                   ...
                   $i = 0;
                        foreach($result[0] as $item_result) {
                        if($i > 0 ) { 
                            $start='AAA">';
                            $end='</a';
                            $dataNeed = PartitionString($start, $end, $item_result);
                            var_export($dataNeed);
                            /*$dataNeed = explode('</tr>',$item_result); 
                                $dataNeed = $dataNeed[1];
                                $dataNeed = explode('</td>',$dataNeed);
                                 $nameNeed =explode("alt='",$dataNeed[2]);
                                 $nameNeed = $nameNeed[1];
                                 $nameNeed = explode("' src=",$nameNeed);
                   */
                            ...
                            //die();
                                ...
                                $dataNeed1 = explode('</tr>',$item_result[3]);
                                $dataNeed1 = $dataNeed1[3];
                                $dataNeed1 = explode('</td>',$dataNeed1);
                                $dataFinal1 = array();
                                foreach($dataNeed as $dN){
                                    $dN = strip_tags($dN);
                                    $dataFinal[] = preg_replace('/\s+/', ' ', trim($dN));
                                }
                                 foreach($dataNeed1 as $dN1){
                                    $dN1 = strip_tags($dN1);
                                    $dataFinal1[] = preg_replace('/\s+/', ' ', trim($dN1));    
                                }
                                if(isset($dataFinal[1])){
                                    $ceo = $dataFinal[1];
                                    $age=$dataFinal1[3];
                                ...
                                //pre($dataUpdate);die();
                                $this->db->insert('all_women_caf_age_tmp',$dataInsert);
                                */
                        }
                        $i++;
                    }
