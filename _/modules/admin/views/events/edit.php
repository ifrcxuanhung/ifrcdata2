...
<article class="no-margin">
    <form action="" method="post" class="form">
        <section id="box-info" style="width:55% !important" class="grid_5"<?php if(isset($input)) echo ' style="display: none;"'; ?>>
            <div class="block-border">
                ...
                <h1>Events</h1>
                    ...
                    ?>
                                <div class="block-controls-2" >
                                    <div class="fx_bt_indi" style="padding-top: 8px;">
                                        ...
                                        <?php
                                            if($info['market'] == 'HSX'){
                                                if(file_exists("\\\\LOCAL\\IFRCVN\\VNDB\\METASTOCK\\CORPORATE ACTIONS\\FILE\\".$data_split)){
                                                    echo '<a href="file://///LOCAL/IFRCVN/VNDB/METASTOCK/CORPORATE ACTIONS/FILE/'.$data_split.'" target="_blank">'.$data_split.'</a>';
                                                }else{
                    ...
                    <?php
                                ...
                                echo '<textarea name="dividends-text" id="dividends-text" style="height:355px; width: 98%">'.$info['content'].'</textarea>';
                            }
                        }else{
                            echo '<textarea name="dividends-text" id="dividends-text" style="height:320px; width: 98%"></textarea>';
                        }
                    ?>
                </div>
            </div>
        </section>
        <section id="box-detail" style="width:40% !important" class="grid_5"<?php if(isset($input)) echo ' style="margin-left: 30%"'; ?>>
                            ...
                            <?php
                                ...
                                foreach($info_type as $it){
                                    if(in_array($it['id'],$info_check)){
                                        $i++;
                                        $href = str_replace(' ','_',$it['evname_en']);
                            ?>
                            <li><a id="" class="get_table" href="#<?= $href ?>"><?= $it['evname_en'] ?></a></li>
                            <?php
                                     }
                                }
                            ?>
                        ...
                        <div class="tabs-content">
                            ...
                            <div id="<?= $href ?>">
                                <ul class="blocks-list">
                                    <?php
                                        foreach($it['field'] as $field){
                                    ?>
                                    <li>
                                        <input type="hidden" name="<?= $href.'_ticker' ?>" value="<?= $info['ticker'] ?>" />
                                        <input type="hidden" name="<?= $href.'_market' ?>" value="<?= $info['market'] ?>" />
                                        <input type="hidden" name="<?= $href.'_date_ann' ?>" value="<?= $info['date_ann'] ?>" />
                                        <input type="hidden" name="type[]" value="<?= $it['evname_en'] ?>" />
                                        <a class="float-left" href="#"><img width="16" height="16" src="http://local/vnfdb/assets/templates/backend/images/icons/fugue/status.png"><?= trans($field['name']) ?></a>
                                        <div class="columns">
                                            <p class="colx2-right">
                                                <?php
                                                    if($field['format']){
                                                        $class = 'class="datepicker1 full-width"';
                                                    }else{
                                                        $class = 'class="full-width"';
                                                    }
                                                    if($field['name'] == 'ticker'){
                                                        echo '<input type="text" '.$class.' value="'.$info['ticker'].'" name="'.$href.'_'.$field['name'].'" disabled>';
                                                    }elseif($field['name'] == 'market'){
                                                        echo '<input type="text" '.$class.' value="'.$info['market'].'" name="'.$href.'_'.$field['name'].'" disabled>';
                                                    }elseif($field['name'] == 'date_ann'){
                                                        echo '<input type="text" '.$class.' value="'.$info['date_ann'].'" name="'.$href.'_'.$field['name'].'" disabled>';
                                                    }else{
                                                        if(!isset($info[$it['evname_en']][$field['name']])){
                                                            $info[$it['evname_en']][$field['name']] = '';
                                                        }
                                                        echo '<input type="text" '.$class.' value="'.$info[$it['evname_en']][$field['name']].'" name="'.$href.'_'.$field['name'].'"';
                                                    }
                                                ?>
                                            </p>
                                        </div>
                                ...
                                <div style="text-align: right;">
                                    <a href="<?= admin_url() ?>events/add/<?= $info['id'] ?>" id="submit">Back</a>
